...
Blockly.Toolbox.prototype.getWidth = function() {
  window.localStorage.setItem("ToolboxWidth", this.width);
  return this.width;
};
...
Blockly.Toolbox.prototype.updateSelectedItemColour_ = function(tree) {
  ...
  if (selectedItem) {
    ...
    selectedItem.getRowElement().style.borderColor = hexColour;
  ...
  for (var i = 0, child; child = children[i]; i++) {
    ...
    if (element) {
      ...
      if (toolboxOptions.inverted) {
        if (this.hasColours_) {
          element.style.color = (child.hexColour || '#333');
            element.style.backgroundColor = ('#FFFFFF');
          element.style.borderColor = (child.hexColour || '#fa0202');
          ...
          if (!child.disabled) {
            ...
            Blockly.bindEvent_(child.getRowElement(), 'mouseenter', child,
                function(/*e*/) {
                  if (!this.isSelected()) {
                    this.getRowElement().style.backgroundColor = ('#eee');
                    this.getRowElement().style.borderColor = Blockly.PXTUtils.fadeColour(this.hexColour || '#ddd', invertedMultiplier, false);
                  }
                });
            Blockly.bindEvent_(child.getRowElement(), 'mouseleave', child,
                function(/*e*/) {
                  if (!this.isSelected()) {
                    this.getRowElement().style.background = ('#FFF');
                    this.getRowElement().style.borderColor = Blockly.PXTUtils.fadeColour(this.hexColour || '#ddd', invertedMultiplier, false);
                  }
                });
          }
...
Blockly.Toolbox.TreeControl.prototype.setSelectedItem = function(node) {
  ...
  if (toolbox.lastCategory_ && toolbox.isInverted_()) {
    toolbox.lastCategory_.getRowElement().style.backgroundColor = '#FFF';
  }
  if (node) {
