...
Blockly.Arduino['math_constant'] = function(block) {
  ...
  var CONSTANTS = {
    'PI': ['math.pi', Blockly.Arduino.ORDER_NONE],
    'E': ['math.e', Blockly.Arduino.ORDER_NONE],
    'GOLDEN_RATIO': ['(1 + math.sqrt(5)) / 2',
                     Blockly.Arduino.ORDER_NONE],
    'SQRT2': ['math.sqrt(2)', Blockly.Arduino.ORDER_NONE],
    'SQRT1_2': ['math.sqrt(1.0 / 2)', Blockly.Arduino.ORDER_NONE],
    'INFINITY': ['float(\'inf\')', Blockly.Arduino.ORDER_NONE]
  };
...
Blockly.Arduino['math_number_property'] = function(block) {
  ...
  if (dropdown_property == 'PRIME') {
    ...
    var functionName = Blockly.Arduino.provideFunction_(
        'math_isPrime',
        ['def ' + Blockly.Arduino.FUNCTION_NAME_PLACEHOLDER_ + '(n):',
  ...
  switch (dropdown_property) {
    ...
    case 'DIVISIBLE_BY':
      var divisor = Blockly.Arduino.valueToCode(block, 'DIVISOR',
          Blockly.Arduino.ORDER_NONE);
      // If 'divisor' is some code that evals to 0, Arduino will raise an error.
      if (!divisor || divisor == '0') {
        return ['False', Blockly.Arduino.ORDER_NONE];
      }
  ...
  switch (func) {
    ...
    case 'MODE':
      var functionName = Blockly.Arduino.provideFunction_(
          ...
          ['def ' + Blockly.Arduino.FUNCTION_NAME_PLACEHOLDER_ + '(some_list):',
           '  modes = []',
    ...
    case 'STD_DEV':
      ...
      var functionName = Blockly.Arduino.provideFunction_(
          ...
          ['def ' + Blockly.Arduino.FUNCTION_NAME_PLACEHOLDER_ + '(numbers):',
           '  n = len(numbers)',
           '  if n == 0: return',
...
Blockly.Arduino['math_constrain'] = function(block) {
  ...
  var argument2 = Blockly.Arduino.valueToCode(block, 'HIGH',
      Blockly.Arduino.ORDER_NONE) || 'float(\'inf\')';
...
Blockly.Arduino['math_random_float'] = function(block) {
  ...
  Blockly.Arduino.definitions_['import_random'] = 'import random';
  return ['random.random()', Blockly.Arduino.ORDER_NONE];
};
...
Blockly.Arduino['math_whole_number'] = function(block) {
  // Numeric value.
  var code = parseFloat(block.getFieldValue('NUM'));
  var order = code >= 0 ? Blockly.Arduino.ORDER_NONE :
              Blockly.Arduino.ORDER_NONE;
  return [code, order];
};
